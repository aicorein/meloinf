...
async def weather(adapter: Adapter, args: CmdArgs) -> None:
    ...
    async with _send_req(CITY_LOOKUP_URL, city) as resp:
        if resp.status != 200:
            ...
            await adapter.send_reply("城市 id 查询失败...请稍后再试，或联系 bot 管理员解决")
            return
        res = await resp.json()
    ...
    async with _send_req(WEATHER_NOW_URL, city_id) as resp:
        if resp.status != 200:
            ...
            await adapter.send_reply("城市当前天气获取失败...请稍后再试，或联系 bot 管理员解决")
            return
        res = await resp.json()
    ...
    async with _send_req(WEATHER_7D_URL, city_id) as resp:
        if resp.status != 200:
            ...
            await adapter.send_reply("城市多天天气获取失败...请稍后再试，或联系 bot 管理员解决")
            return
        res = await resp.json()
    ...
    hg = await adapter.send(ImageSendSegment(file=fig_b64))
    await hg
